...
def train(epoch):
	...
	for batch_idx,(content_images_batch,_) in enumerate(content_images_loader):
		...
		if gpu:
			...
			content_images_batch_var = Variable(content_images_batch.cuda(), requires_grad=False)
		else:
			input_images_batch = Variable(input_images_batch)
			content_images_batch_var = Variable(content_images_batch, requires_grad=False)
...
def predict(e=0,b=0):
	...
	imshow(output.data, 'p'+str(e)+'_'+str(b))
